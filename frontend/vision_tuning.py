...
_INTEGER_PARAMETERS = {
    ...
    "edge_min_line_length",
    "edge_support_neighborhood",
    "edge_work_max_width",
    "edge_work_max_height",
    "edge_max_candidates",
...
_PARAMETER_RANGES: dict[str, tuple[float, float, float]] = {
    ...
    "edge_operator": (0.0, 3.0, 1.0),
    "edge_gradient_threshold": (1.0, 255.0, 1.0),
    "edge_anchor_threshold": (0.0, 255.0, 1.0),
    "edge_scan_interval": (1.0, 10.0, 1.0),
    "edge_min_path_length": (2.0, 200.0, 1.0),
    "edge_min_line_length": (-1.0, 200.0, 1.0),
    "edge_sigma": (0.1, 5.0, 0.1),
    "edge_line_fit_error": (0.1, 10.0, 0.1),
    "edge_max_line_gap": (0.0, 50.0, 0.5),
    "edge_max_error": (0.1, 10.0, 0.1),
    "min_radius": (1.0, 300.0, 1.0),
    "max_radius": (2.0, 300.0, 1.0),
    "edge_min_circle_ratio": (0.0, 1.0, 0.01),
    "edge_min_support_ratio": (0.0, 1.0, 0.01),
    "edge_support_neighborhood": (0.0, 8.0, 1.0),
    "edge_min_visible_ratio": (0.0, 1.0, 0.01),
    "edge_work_max_width": (160.0, 3840.0, 20.0),
    "edge_work_max_height": (120.0, 2160.0, 20.0),
    "edge_max_candidates": (1.0, 500.0, 1.0),
    "expected_radius": (0.0, 300.0, 1.0),
    "expected_radius_tolerance_ratio": (0.0, 1.0, 0.01),
    "diameter_min_visible_ratio": (0.0, 1.0, 0.01),
...
class TuningWindow(QWidget):
    ...
    def _controls_for_stage(self, index: int) -> list[dict[str, Any]]:
        ...
        controls: dict[int, list[dict[str, Any]]] = {
            ...
            8: [
                self._check("enable_edge_clahe", "启用 CLAHE", config.enable_edge_clahe),
                self._check("enable_edge_median_blur", "启用中值滤波", config.enable_edge_median_blur),
                self._number("edge_work_max_width", "最大工作宽度", config.edge_work_max_width),
                self._number("edge_work_max_height", "最大工作高度", config.edge_work_max_height),
            ],
            9: [
                self._number("edge_operator", "梯度算子（0–3）", config.edge_operator),
                self._number("edge_gradient_threshold", "梯度阈值", config.edge_gradient_threshold),
                self._number("edge_anchor_threshold", "锚点阈值", config.edge_anchor_threshold),
                self._number("edge_scan_interval", "扫描间隔", config.edge_scan_interval),
                self._number("edge_min_path_length", "最短边缘路径", config.edge_min_path_length),
                self._number("edge_sigma", "高斯 Sigma", config.edge_sigma),
            ],
            10: [
                self._number("edge_min_line_length", "最短拟合线（-1=自动）", config.edge_min_line_length),
                self._number("edge_line_fit_error", "直线拟合误差", config.edge_line_fit_error),
                self._number("edge_max_line_gap", "最大线段间距", config.edge_max_line_gap),
                self._number("edge_max_error", "最大拟合误差", config.edge_max_error),
                self._check("edge_nfa_validation", "启用 NFA 验证", config.edge_nfa_validation),
                self._check("edge_pf_mode", "启用参数自由模式", config.edge_pf_mode),
                self._number("edge_min_circle_ratio", "最小近圆比", config.edge_min_circle_ratio),
            ],
            11: [
                self._number("min_radius", "最小半径", config.min_radius),
                self._number("max_radius", "最大半径", config.max_radius),
                self._number("edge_min_support_ratio", "最小边缘支撑", config.edge_min_support_ratio),
                self._number("edge_support_neighborhood", "边缘支撑邻域", config.edge_support_neighborhood),
                self._number("edge_min_visible_ratio", "最小可见圆周", config.edge_min_visible_ratio),
                self._number("edge_max_candidates", "最大候选数", config.edge_max_candidates),
                ...
                self._number("expected_radius", "期望半径（px）", config.expected_radius),
                self._number("expected_radius_tolerance_ratio", "半径容差比例", config.expected_radius_tolerance_ratio),
                self._number("diameter_min_visible_ratio", "直径有效完整度", config.diameter_min_visible_ratio),
            ],
        }
