...
- `detector.py`：分级液滴检测模块；在缩小图上以自适应二值、背景差分和连通域生成快速候选，对粘连区域局部 Watershed，对疑难区域局部 Hough，最后回到原分辨率精修并融合评分。
...
工作台只读取本地视频并调用 `DropletDetector`，支持逐帧预览和 JSON 参数保存。视频路径、加载按钮和帧滑块集中在顶部紧凑工具栏中。候选默认由快速分割/连通域主分支产生；规则液滴直接椭圆拟合，粘连区域才运行局部 Watershed，低置信区域才运行局部 Hough。快速候选会映射回原图进行边缘和尺寸精修。

当前帧用多宫格依次展示原图、灰度转换、对比度归一化、输入高斯平滑、缩放检测图、自适应二值、背景运动分割、连通域与边缘、轮廓候选、局部 Watershed、局部 Hough 和融合结果。缩放比例、分割阈值、背景学习率、圆度、长短轴比、Watershed 峰值和 Hough 参数均可在所属卡片直接编辑。所有常用数值参数同时提供算法安全范围内的滑条和精确输入框。

密集液滴场景只在含多个距离峰的局部连通域执行 Watershed；Hough 默认不扫描整帧，仅验证粘连或校验失败区域。若快速分支完全没有候选，则执行一次全帧 Hough 兜底；`hough_refresh_interval` 可设为 2 或 3，按需增加周期性全帧校验，默认 `0` 关闭。最终通过半径、圆度、长短轴比、边缘覆盖、内外亮度和重叠抑制控制误检。跟踪器默认采用 3 帧窗口至少 2 次命中才确认轨迹，确认后短时漏检由 Kalman 预测维持。工作台不会连接相机、泵机，不运行跟踪、磁珠识别或 PID。
