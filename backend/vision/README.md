...
- `detector.py`：液滴检测模块；对有效管道区域执行灰度转换、大尺度高斯背景校正、CLAHE 和高斯平滑，再以 `cv2.HoughCircles` 直接输出液滴圆，并按从上到下、从左到右稳定排序。
...
工作台只读取本地视频并调用 `DropletDetector`，支持逐帧预览和 JSON 参数保存。视频路径、加载按钮和帧滑块集中在顶部紧凑工具栏中。当前算法只保留整帧 Hough 圆检测，不再执行轮廓、背景差分、连通域、Watershed、候选融合、边缘支撑过滤、尺寸门控或亮度评分。默认半径范围为 18–32 px、最小圆心距离为 32 px、敏感度为 0.96；敏感度按 `param2 = 45 - 25 × sensitivity` 映射到 Hough 累加阈值。
