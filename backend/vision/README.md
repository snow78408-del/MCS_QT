...
## 3. 每个文件功能
- `config.py`：统一管理参数（EdgeDrawing、圆形筛选、ROI、匹配距离、最大未匹配帧数、磁珠面积、debug 开关、tracker 类型、Kalman 参数等）。
...
- `detector.py`：EdgeDrawing 圆形液滴检测模块；在有效管道区域提取连接边缘，只接受原生圆和近圆记录，再按半径、近圆比、可见圆周、边缘支撑和中心距离过滤。输出仅包含圆心和半径，直接供现有跟踪/控制接口使用。
...
- `detector`：从有效管道区域的单帧中得到候选圆形液滴（圆心和半径）。
...
工作台只读取本地视频并调用 `DropletDetector`，支持逐帧预览和 JSON 参数保存。视频路径、加载按钮和帧滑块集中在顶部紧凑工具栏中。液滴算法只使用 EdgeDrawing 边缘提取并输出圆形候选，不执行 Hough 圆、轮廓、背景差分、连通域、Watershed 或候选融合。

当前帧先用四个大卡片展示“管道区域检定”：原始大图、局部高频区域、高低频界线拟合和最终有效区域/可信度；之后再依次展示灰度转换、对比度归一化、高斯平滑、EdgeDrawing 输入预处理、连接边缘图、原始圆候选和过滤结果。管道检定和液滴检测参数均可直接调整，检定开关关闭时会明确显示“已跳过”。保存文件采用分组 JSON，包含 `channel_region` 与 `detector` 两组参数。旧版液滴检测配置不再兼容。
